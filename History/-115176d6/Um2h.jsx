...
const CartProduct = () => {
  ...
  const getProductUserCart = async () => {
    try {
      ...
      response.data.forEach((cartProduk) => {
        ...
        if (cartProduk.cartProduct_id.image && cartProduk.cartProduct_id.image.data) {
          const buffer = cartProduk.cartProduct_id.image.data;
  ...
  const deleteProductCart = async (infocart_id) => {
    try {
      // console.log("cartproductid" + infocart_id);
      // console.log("userid" + user_id);
      await axios.delete(`http://localhost:5000/cart/${infocart_id}`);
      // getProductUserCart();
      navigate('/cart');
  ...
  return (
    <div className="flex flex-col justify-center space-y-2">
      {infoCart.map((cartProduk) => (
        <div className="px-2 flex flex-row space-x-1 md:space-x-5  space-y-0 rounded-xl max-w-xl w-full  mx-auto border border-white bg-gray-200 shadow-lg hover:shadow-xl hover:scale-105 duration-300">
          <div className="w-1/5  grid place-items-center">
            <Link to={`/product/info/${cartProduk.cartProduct_id.uuid}`}>
              <img
          ...
          <div className="w-full w-4/5  flex flex-col justify-between p-3">
            <Link to={`/product/info/${cartProduk.cartProduct_id.uuid}`}>
              <h3 className="font-bold text-gray-800 text-2xl">
                {cartProduk ? cartProduk.cartProduct_id.name : ''}
              </h3>
            </Link>
            <p className="text-sm font-semibold ">
              Rp. {cartProduk ? cartProduk.cartProduct_id.price : ''}
            </p>
            <div className="h-1/5 flex justify-end items-center">
              <button
                onClick={() => deleteProductCart(cartProduk._id)}
                type="submit"
