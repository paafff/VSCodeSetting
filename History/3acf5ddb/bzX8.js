  ...
  upload(req, res, async (error) => {
    ...
    try {
      ...
      const imgFilesOne = req.files['imgFilesOne']
        ? await sharp(req.files['imgFilesOne'][0].buffer).png().toBuffer()
        : undefined;
      const imgFilesTwo = req.files['imgFilesTwo']
        ? await sharp(req.files['imgFilesTwo'][0].buffer).png().toBuffer()
        : undefined;
      const imgFilesThree = req.files['imgFilesThree']
        ? await sharp(req.files['imgFilesThree'][0].buffer).png().toBuffer()
        : undefined;
...
export const getArticles = async (req, res) => {
  ...
  if (findArticleImg.img1) {
    articleObj.img1 = findArticleImg.img1;
  }
  if (findArticleImg.img2) {
    articleObj.img2 = findArticleImg.img2;
  }
  if (findArticleImg.img3) {
    articleObj.img3 = findArticleImg.img3;
  }
