...
export const loginUser = async (req, res) => {
  ...
  res
    .status(200)
    .json({ _id, uuid, name, email, address, phone, gender, role });
};

export const getMe = async (req, res) => {
  if (!req.session.temporarySession) {
    return res.status(401).json({ msg: "harap login dulu..." });
  }

  const findUser = await userDb
    .findOne({ uuid: req.session.temporarySession })
    .select("_id uuid name email address image phone gender role ");
