...
export const createProduct = async (req, res) => {
  upload(req, res, async (error) => {
    ...
    // maka akan terjadi eror, oleh sebab itu kita gunakan atau kuita define langsung pada trycatch blok dibawah dengan tertnary
...
export const updateProduct = async (req, res) => {
  upload(req, res, async (error) => {
    ...
    try {
      ...
      const { name, price, description } = req.body;
      const updateField = {
        name: name || filterProductUser.name,
        price: price || filterProductUser.price,
        description: description || filterProductUser.description,
        image: req.file
          ? await sharp(req.file.buffer)
              .jpeg()
              .resize({ width: 400, height: 400, fit: 'inside' })
